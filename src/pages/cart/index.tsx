...
const Cart: React.FC = () => {
	...
	useEffect(() => {
		...
		setBookList(cartContext.cartData.result);
		setItemsInCart(cartContext.cartData.result.length);
		getTotalPrice(cartContext.cartData.result);
	}, [cartContext.cartData.result, cartContext.cartData.totalRecords]);
	...
	const PlaceOrder = async () => {
		if (authContext.user.id) {
			...
			if (userCart.result.length) {
				try {
					let cartIds: number[] = [];
					userCart.result.forEach((element: CartList) => {
